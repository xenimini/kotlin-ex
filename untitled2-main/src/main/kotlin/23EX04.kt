// 1.1 b) Добавление 3d сцены из fun main()

// 1.2 a) Выполняет полученное событие

// 1.3 d) Указывает, что значение строго не null

// 1.4 c) kotlin это зумер, java это бумер
...
fun main() = KoolApplication{
    ...
    // вращалочка
    val cubeMoving = mutableStateOf(false)
    ...
    addScene {
        ...
        addColorMesh {
            ...
            onUpdate{
                if (cubeMoving.value) {
                    transform.rotate(360f.deg * Time.deltaT, Vec3f.Z_AXIS)
                }
            }
        }
    ...
    addScene {
        ...
        addPanelSurface {
            ...
            Column {
                ...
                //---------------- ЗАДАНИЕ 2 ----------------
                val hpPercent = derivedStateOf {
                    (game.hp.value.toFloat() / 100f).coerceIn(0f, 1f)
                }
                Box(
                    modifier = Modifier
                        .width(200.dp)
                        .height(20.dp)
                        .margin(bottom = 8.dp)
                        .background(RoundRectBackground(Color(0.3f, 0.3f, 0.3f, 1f), 10.dp))
                ) {
                    Box(
                        modifier = Modifier
                            .width((200 * hpPercent.value).dp)
                            .height(20.dp)
                            .background(
                                RoundRectBackground(
                                    color = Color(
                                        r = (1f - hpPercent.value).coerceIn(0f, 1f),
                                        g = hpPercent.value.coerceIn(0f, 1f),
                                        b = 0f,
                                        a = 1f
                                    ),
                                    radius = 10.dp
                    ...
                    Text(
                        text = "${(hpPercent.value * 100).toInt()}%",
            ...
            Row {
                ...
                Button("вращать кубик") {
                    modifier.onClick {
                        cubeMoving.value = true
                    }
                }
                Button("прекратить вращенение") {
                    modifier.onClick {
                        cubeMoving.value = false
                    }
                }
            }
