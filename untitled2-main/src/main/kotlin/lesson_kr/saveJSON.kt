...
class ServerWorld(
    ...
    fun dealDamage(playerId: String, targetId: String, amount: Int){
        ...
        val newHp = (old.hp - amount).coerceAtLeast(0) // чтоб не уходило в минус
    ...
    suspend fun applyLoaded(playerSave: PlayerSave) {
        // Заменяем старое состояние на загруженное
        _playerState.value = playerSave

        // Публикуем событие о загрузке
        _events.emit(PlayerProgressSaved(playerSave.playerId, "загрузка файла вручную из JSON"))
    }
}
            ...
            Row {
                ...
                Button("Загрузить JSON") {
                    modifier.onClick {
                        ...
                        if (loaded != null) {
                            this@addScene.coroutineScope.launch {
                                // Применяем загруженное состояние к серверу
                                server.applyLoaded(loaded)
                                uiLog(ui, "Загружено сохранение игрока $pid из файла")
                            }
                        } else {
                            uiLog(ui, "Не удалось загрузить сохранение для $pid")
                        }
                    }
                }
